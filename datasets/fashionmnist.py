...
class Dataset(base.ImageDataset):
    ...
    def get_train_set(use_augmentation):
        ...
        train_set = torchvision.datasets.FashionMNIST(
            train=True, root=os.path.join(get_platform().dataset_root, 'fashionmnist'), download=True)
        return Dataset(train_set.data, train_set.targets)

    @staticmethod
    def get_test_set():
        test_set = torchvision.datasets.FashionMNIST(
            train=False, root=os.path.join(get_platform().dataset_root, 'fashionmnist'), download=True)
