...
class Dataset(base.ImageDataset):
    ...
    def get_train_set(use_augmentation):
        # No augmentation for KMNIST.
        train_set = torchvision.datasets.KMNIST(
            train=True, root=os.path.join(get_platform().dataset_root, 'kmnist'), download=True)
        return Dataset(train_set.data, train_set.targets)

    @staticmethod
    def get_test_set():
        test_set = torchvision.datasets.KMNIST(
            train=False, root=os.path.join(get_platform().dataset_root, 'kmnist'), download=True)
        return Dataset(test_set.data, test_set.targets)

    def __init__(self,  examples, labels):
        tensor_transforms = [torchvision.transforms.Normalize(mean=[0.286], std=[0.3205])]
